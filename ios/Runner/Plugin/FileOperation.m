...
+(BOOL)createFileAtPath:(NSString *)path content:(NSObject *)content overwrite:(BOOL)overwrite error:(NSError *__autoreleasing *)error {
    ...
    if (content) {
        [self writeFileAtPath:path content:content encode:@"utf-8" error:error];
    }
    return isSuccess;
}
...
+(BOOL)writeFileAtPath:(NSString *)path content:(NSObject *)content encode:(NSString *)encode {
    return [self writeFileAtPath:path content:content encode:encode error:nil];
}

+(BOOL)writeFileAtPath:(NSString *)path content:(NSObject *)content encode:(NSString *)encode error:(NSError *__autoreleasing *)error {
    if (!content) {
    ...
    if ([self isExistsAtPath:path]) {
        if ([content isKindOfClass:[NSMutableArray class]]) {
            ...
            if([encode isEqualToString:@"gbk"])
                [[(NSString *)content dataUsingEncoding:kCFStringEncodingGB_18030_2000] writeToFile:path atomically:YES];
            else
