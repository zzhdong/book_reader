...
/// 覆盖动画
class AnimationCover extends BaseAnimation {

  AnimationCover() : super();
  ...
  void _drawCurrentShadow(Canvas canvas) {
    canvas.save();
    if (isTurnNext) {
      double dx = currentSize.width + mTouchOffset.dx - mStartOffset.dx;
      canvas.drawShadow(
          Path()
            ..moveTo(dx, 0)
            ..lineTo(dx + 5, 0)
            ..lineTo(dx + 5, currentSize.height)
            ..lineTo(dx, currentSize.height)..close(),
          Colors.black, 20, false);
    }else{
      double dx = mTouchOffset.dx - mStartOffset.dx;
      canvas.drawShadow(
          Path()
            ..moveTo(dx, 0)
            ..lineTo(dx - 5, 0)
  ...
  bool isBeginArea(Offset touchPos) {
    ...
    if(touchPos.dx == mStartOffset.dx && touchPos.dy == mStartOffset.dy) {
      ...
      return (mTouchOffset.dx - mStartOffset.dx).abs() < 20;//(currentSize.width / 4);
    }
  }
