...
class HSVColorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      ...
      size.height * 0.04,
      Paint()
        ..color = ColorUtils.useWhiteForeground(hsvColor.toColor()) ? Colors.white : Colors.black
        ..strokeWidth = 1.5
...
class HSLColorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      ...
      size.height * 0.04,
      Paint()
        ..color = ColorUtils.useWhiteForeground(hslColor.toColor()) ? Colors.white : Colors.black
        ..strokeWidth = 1.5
        ..style = PaintingStyle.stroke,
    );
  }
...
class ColorPickerLabel extends StatefulWidget {
  const ColorPickerLabel(this.hsvColor, {super.key});
...
class _ColorPickerLabelState extends State<ColorPickerLabel> {
  ...
  List<String> colorValue(HSVColor hsvColor, ColorModel colorModel) {
    ...
    if (colorModel == ColorModel.hex) {
      ...
      HSLColor hslColor = ColorUtils.hsvToHsl(hsvColor);
      return [
        '${hslColor.hue.round()}°',
        ...
        child: IntrinsicHeight(
          child: Column(
            children: <Widget>[
              Text(
                item,
                style: Theme.of(context).textTheme.bodySmall!.copyWith(fontWeight: FontWeight.bold, fontSize: 16.0),
              ),
...
class ColorPickerSlider extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints box) {
      double thumbOffset = (box.maxWidth - 30.0) * (trackType == TrackType.hue ? hsvColor.hue / 360 : hsvColor.toColor().opacity) + 15.0;
            ...
            child: Transform.translate(
              ...
              child: CustomPaint(
                painter: ThumbPainter(
                  thumbColor: displayThumbColor ? trackType == TrackType.hue ? HSVColor.fromAHSV(1.0, hsvColor.hue, 1.0, 1.0).toColor() : Colors.black.withOpacity(hsvColor.alpha) : null,
                ),
              ),
            ),
          ),
              ...
              builder: (BuildContext context, BoxConstraints box) {
                return GestureDetector(
                  onPanUpdate: (DragUpdateDetails details) {
                    ...
                    double localDx = getBox.globalToLocal(details.globalPosition).dx - 15.0;
                    if (trackType == TrackType.hue) {
                      onColorChanged(hsvColor.withHue(localDx.clamp(0.0, box.maxWidth - 30.0) / (box.maxWidth - 30.0) * 360));
  ...
  void _handleColorChange(double horizontal, double vertical) {
    switch (paletteType) {
      ...
      case PaletteType.hsl:
        onColorChanged(ColorUtils.hslToHsv(ColorUtils.hsvToHsl(hsvColor).withSaturation(horizontal).withLightness(vertical)));
        break;
    }
  }
          ...
          child: Builder(
            builder: (BuildContext _) {
              switch (paletteType) {
                ...
                case PaletteType.hsl:
                  return CustomPaint(painter: HSLColorPainter(ColorUtils.hsvToHsl(hsvColor)));
                default:
...
class ColorPickerInput extends StatefulWidget {
  const ColorPickerInput(this.hsvColor, this.onColorChanged, {super.key});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: TextField(
        ...
        controller: TextEditingController(text: ColorUtils.toHex(widget.hsvColor.toColor())),
        style: const TextStyle(fontSize: 17.0, color: Colors.black87, fontWeight: FontWeight.w700),
        decoration: const InputDecoration(
          contentPadding: EdgeInsets.fromLTRB(0, 14, 0, 14),
          border: InputBorder.none,
        ),
        onChanged: (value) {
          if (StringUtils.startWithIgnoreCase(value, "#")) {
            if (value.length == 7) widget.onColorChanged(HSVColor.fromColor(ColorUtils.fromHex(value)));
          } else {
            if (value.length == 6) widget.onColorChanged(HSVColor.fromColor(ColorUtils.fromHex(value)));
          }
        },
      ),
    );
